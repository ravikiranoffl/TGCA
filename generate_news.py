...
# --- 11. DATE FORMATTING & TELUGU TRANSLATION FOR EMAIL ---
    ...
    # Find Section 26 (The English Summary)
    summary_start = content.find("## SECTION 26")
    
    if summary_start != -1:
        # 1. Extract the English Summary text
        english_summary = content[summary_start + 13:].strip()
        
        # 2. Ask Gemini for a quick Telugu Translation of that summary
        telugu_prompt = f"Translate this news summary into professional Telugu (Vaarthalu style). Keep the bullet points:\n\n{english_summary}"
        # Using the same 'client' and 'model' you initialized at the start of your script
        telugu_response = client.models.generate_content(model='gemini-2.5-flash', contents=telugu_prompt)
        telugu_text = telugu_response.text

        # 3. Format as a beautiful HTML Email
        # We replace Markdown bullets with HTML tags for Gmail
        ...
        final_html = f"<html><body><p>News Briefing for {display_date} is ready in your repository.</p></body></html>"

    # Save the HTML file for the YAML to pick up
